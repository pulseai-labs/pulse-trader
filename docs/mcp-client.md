...
|---|---|---|
| `list_strategies` | `{ include_archived?: bool }` | strategy/version tree, parent-first |
| `get_version` | `{ version_id }` | migrated DSL document, verbatim original, hash, provenance |
| `list_runs` | `{ version_id }` | runs against the version, headline stats + input provenance |
| `get_run` | `{ run_id }` | summary stats, regime breakdown, skipped entries, MFE/MAE, persisted inputs |
