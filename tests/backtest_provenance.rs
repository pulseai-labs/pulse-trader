...
async fn migration_0006_applies_through_the_startup_path_despite_0007() {
    ...
    // maximum to 10; r2.s2.w2's `0011` rides along likewise, to 11; and
    // r2.s3.w2's `0012` (window lead-in) moves it to 12.
    ...
    assert_eq!(
        applied.iter().copied().max(),
        Some(12),
        "0006 is recorded at its own version, below the maximum 0012 sets"
    );
