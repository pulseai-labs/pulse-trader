...
async fn run_backtest_refused_when_an_htf_child_inherits_an_m15_only_run() {
    ...
    // The schema-1.1.0 child: the parent's shape plus one `htf` operand.
    let mut child_doc: Value = serde_json::from_str(MINIMAL_DSL).expect("MINIMAL_DSL parses");
    child_doc["schema_version"] = json!("1.1.0");
    child_doc["name"] = json!("RSI Oversold (mcp htf child)");
    child_doc["entry"] = json!({
        "type": "Compare",
    ...
    let child = strategies
        .create_version(NewVersion {
            ...
            dsl_json: serde_json::to_string(&child_doc).expect("child doc serializes"),
    ...
    let fixture = Fixture {
        ...
        db_path,
        store_dir,
        seed: (parent.id, child.id.clone(), parent_run),
    };
