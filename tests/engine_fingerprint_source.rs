...
//!   vi.  `ENGINE_SOURCE_ROOTS` is the literal nine-root list the spec locks
...
/// Creates all nine locked roots under `base` — the four directories empty,
/// the five file roots as files holding `body` — so `hash` can run on a
